...
export default function StudentCourses() {
  ...
  const fetchCourses = async () => {
    try {
      setLoading(true);
      const data = await apiCall("/api/student/courses");
  ...
  if (loading) return <div style={{ padding: 40, color: C.sub }}>Loading academic courses...</div>;
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div style={{
          ...
          <div>
            <Badge label={`Faculty: ${selectedCourse.faculty || selectedCourse.faculty_name || 'TBD'}`} color="#fff" />
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 8, background: "#fff", padding: 4, borderRadius: 12, border: `1.5px solid ${C.border}`, alignSelf: "flex-start" }}>
          {[
            ...
            { id: "students", label: "Peers List", icon: Icons.Users },
            { id: "resources", label: "Shared Notes", icon: Icons.FileText }
          ].map(tab => {
            const Icon = tab.icon;
        ...
        {activeTab === "resources" && (
          <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 20 }} className="cb-grid-sidebar">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <h4 style={{ fontSize: 14.5, fontWeight: 800, color: C.text }}>Class Resources & Notes</h4>
              {resources.map(res => (
                <Card key={res.id} p={18} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  ...
                  <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 10 }} className="cb-flex-between">
                    ...
                    <button style={{
                      ...
                      <Icons.Download size={13} color={C.primary} /> Access Notes
                    </button>
                  </div>
                </Card>
              ))}
            ...
            <Card style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <h4 style={{ fontSize: 14.5, fontWeight: 800, color: C.text }}>Share Notes / Document</h4>
              <Input label="Title of Note / PDF" placeholder="e.g. Lecture 2 complexity analysis" value={resTitle} onChange={setResTitle} />
              <div className="cb-input-wrap">
                <label className="cb-label">Description / Summary</label>
                <textarea
                  placeholder="Key concepts or drive resource link..."
                  rows={4}
                  value={resDesc}
              ...
              <Btn onClick={handleShareResource} style={{ width: "100%" }}>
                Share Note
              </Btn>
            </Card>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 20 }}>
        {courses.map((c) => (
          <div 
            key={c.id} 
            style={{ 
              background: '#fff', border: '1.5px solid #F4F4F4', borderRadius: 16, padding: 20, 
              boxShadow: '0 4px 12px rgba(0,0,0,0.02)', cursor: "pointer", transition: "transform 0.15s, box-shadow 0.15s"
            }}
            onMouseEnter={e => {
              e.currentTarget.style.boxShadow = "0 6px 16px rgba(108, 99, 255, 0.08)";
          ...
          >
            ...
            <div style={{ fontSize: 13, color: C.sub, marginBottom: 14 }}>Faculty: <b>{c.faculty || c.faculty_name || 'TBD'}</b> · {c.credits} Credits</div>
